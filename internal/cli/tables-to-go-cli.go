...
func (app *App) createTableStructString(table *database.Table) (string, string, error) {
	...
	app.generateHeader(&fileContent, table.Name)
...
func (app *App) generateHeader(content *strings.Builder, tableName string) {
	if !app.settings.GenHeader {
		return
	}

	// Handling any control characters, because apparently all DBs allow them
	// as table names.
	tableName = strconv.Quote(tableName)
	tableName = tableName[1 : len(tableName)-1]

	content.WriteString("// Code generated by tables-to-go. DO NOT EDIT.\n")
	content.WriteString("// versions:\n")
	content.WriteString("// \ttables-to-go")
	content.WriteString(" ")
	content.WriteString(app.settings.GeneratorVersion)
	content.WriteString("\n")
